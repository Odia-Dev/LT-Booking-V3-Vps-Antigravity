...
import { createVehicleSchema, updateVehicleSchema } from "./vehicleValidation";
...
export async function getVehicleBySlug(req: Request, res: Response): Promise<void> {
  try {
    ...
    res.status(404).json({ success: false, message: error.message || "Vehicle not found" });
  }
}
...
export async function updateVehicle(req: Request, res: Response): Promise<void> {
  try {
    ...
    res.status(400).json({ success: false, message: error.message || "Failed to update vehicle" });
  }
}
...
export async function deleteVehicle(req: Request, res: Response): Promise<void> {
  try {
    ...
    res.status(400).json({ success: false, message: error.message || "Failed to delete vehicle" });
  }
}
