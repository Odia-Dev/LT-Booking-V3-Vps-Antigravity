...
export default function EditColorPage() {
  ...
  const fetchColor = useCallback(async () => {
    try {
      ...
      if (!res.ok) throw new Error("Failed to load color details");
      const data = await res.json();
      const color = data.color || data;
      setVehicleId(color.vehicleId);
      setName(color.name);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "Failed to update color");
  ...
  if (loadingColor || loadingVehicles) {
    return <div className="p-16 text-center text-neutral-500 font-mono text-xs uppercase tracking-widest">Loading...</div>;
  }
    ...
    <div className="max-w-2xl mx-auto space-y-8">
      <div className="flex items-center justify-between border-b border-neutral-800 pb-4">
        <div>
          ...
          <h1 className="text-3xl font-black tracking-tight text-white">Edit Color</h1>
        </div>
      </div>
